...
use Concrete\Package\SkybluesofaPageListPlus\Src\PageListPlus\Generator as PageListPlusGenerator;
use Concrete\Package\SkybluesofaPageListPlus\Src\PageListPlus\Filter\Service\Finder as FilterFinder;
...
class Controller extends BlockController
{
    ...
    public $pageTemplateId = array();
    public $parentPageId;
    public $includeAllDescendents;
    public $ignorePermissions;
    public $pageAttributes = array();
    public $pageAttributeIdsUsedInSearch = array();
    public $supportedAttributeTypes = array();
    public $pageAttributeBlacklist = array();
    public $pageAreaBlacklist = array();
    public $resultsRelatedTo = null;
    public $pageAttributesUsedForFilter = array();
    ...
    public function on_start()
    {
        ...
        $al->register('javascript', 'skybluesofa/pagelistplus', 'js/page_list_plus.js', array(), 'skybluesofa_page_list_plus');
        $al->registerGroup('skybluesofa/pagelistplus', array(
            array('javascript', 'jquery'),
            array('javascript', 'core/events'),
            array('javascript', 'skybluesofa/pagelistplus')
        ));
    }
    ...
    public function getFormTabs()
    {
        ...
        foreach ($tabCollections as $title => $tabCollection) {
            $title = t($title);
            $tabs[] = array($tabCollection['id'], $title, $tabCollection['defaultSelected']);
        }
        return $tabs;
    }
    ...
    private function getFormTabCollections()
    {
        if (!$this->_tabCollections) {
            ...
            $tabCollections['Page Selection'] = array('id' => 'plp-page-selection', 'defaultSelected' => true, 'element' => 'page_selection');
            $tabCollections['Search'] = array('id' => 'plp-search', 'defaultSelected' => false, 'element' => 'search');
            $tabCollections['Filters'] = array('id' => 'plp-filters', 'defaultSelected' => false, 'element' => 'filters');
            $tabCollections['Sort'] = array('id' => 'plp-sort', 'defaultSelected' => false, 'element' => 'sort');
            $tabCollections['Display'] = array('id' => 'plp-display', 'defaultSelected' => false, 'element' => 'display');
            /*if ($mapPackage) :
                $tabs[] = array('plp-mapping', t('Mapping'));
            endif;*/
            //$tabCollections[] = array('Copy'=>['id'=>'plp-copy','defaultSelected'=>false]);
            //$tabCollections[] = array('Preview'=>['id'=>'plp-preview','defaultSelected'=>false]);
    ...
    public function setupFormElements()
    {
        ...
        $this->pageTypeId = (!$this->pageTypeId) ? array() : $this->unserialize($this->pageTypeId);
        $this->pageTemplateId = (!$this->pageTemplateId) ? array() : $this->unserialize($this->pageTemplateId);
        $this->pageThemeId = (!$this->pageThemeId) ? array() : $this->unserialize($this->pageThemeId);
        $this->showSearchSelectAsCheckboxAttributes = !$this->showSearchSelectAsCheckboxAttributes ? array() : explode(',', $this->showSearchSelectAsCheckboxAttributes);
        $this->pageAttributeIdsUsedInSearch = !$this->pageAttributeIdsUsedInSearch ? array() : explode(',', $this->pageAttributeIdsUsedInSearch);
        $this->pageAttributesUsedForFilter = !$this->pageAttributesUsedForFilter ? array() : $this->unserialize($this->pageAttributesUsedForFilter);
        $this->searchDefaults = !$this->searchDefaults ? array() : $this->unserialize($this->searchDefaults);
        $this->orderBy = !$this->orderBy ? array('display_asc', 'display_asc', 'display_asc') : $this->unserialize($this->orderBy);
        $this->userSort = !$this->userSort ? array() : $this->unserialize($this->userSort);
        $this->thumbnailHandles = !$this->thumbnailHandles ? array('thumbnail') : $this->unserialize($this->thumbnailHandles);
    ...
    public function normalizeForm($args = array())
    {
        ...
        if (isset($args['pageAttributesUsedForFilter']) && is_array($args['pageAttributesUsedForFilter']) && count($args['pageAttributesUsedForFilter'])) {
            $args['pageAttributeId'] = $args['pageAttributeId'] ? $args['pageAttributeId'] : array();
        ...
        if (isset($args['resultsRelatedTo'])) {
            if (!in_array($args['resultsRelatedTo'], array('keywords', 'area'))) {
                $args['keyword'] = null;
    ...
    {
        $urlFields = array('searchBoxTargetURL', 'seeAllLinkUrl');
    ...
    public function getAttributeFormSearchView($searchFilter = null)
    {
        ...
        return View::element($searchFinder['path'], array('filter' => $searchFilter, 'controller' => $this), $searchFinder['packageHandle']);
    }
}
