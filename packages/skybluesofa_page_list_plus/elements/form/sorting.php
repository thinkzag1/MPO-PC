        ...
        <div class="sbs_plp_searchSort">
            ...
            <select name="orderBy" class="userSorting">
                ...
                <?php   foreach ($controller->getPageAttributes() as $pageAttribute) { ?>
                    <?php  
                    if (in_array('handle_' . $pageAttribute->getAttributeKeyHandle(), $controller->userSort) && in_array($pageAttribute->getAttributeTypeHandle(), array('text', 'textarea', 'boolean', 'date_time', 'number', 'rating', 'select', 'image_file'))) {
                        ?>
                        <option
