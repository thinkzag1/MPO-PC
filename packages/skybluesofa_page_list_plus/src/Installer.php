<?php 
namespace Concrete\Package\SkybluesofaPageListPlus\Src;
...
class Installer extends Object
{
    ...
    //'page_list_plus_additional_classes' => array('name' => 'Additional Classes', 'type' => 'select', 'properties' => array('akIsSearchable' => 1, 'akIsSearchableIndexed' => 1,'akSelectAllowMultipleValues'=>1))
    //];

    protected static $singlePages = array(
        array('path' => '/dashboard/page_list_plus', 'name' => 'Page List+', 'description' => 'Page Lists with keyword and page attribute filtering as well as built-in list titles'),
        array('path' => '/dashboard/page_list_plus/attribute_blacklist', 'name' => 'Attribute Blacklist', 'description' => 'Exclude page attributes from Page List+'),
        array('path' => '/dashboard/page_list_plus/area_blacklist', 'name' => 'Area Blacklist', 'description' => 'Exclude page areas from Page List+'),
    );

    protected static $blockTypes = array(
        array('handle' => 'page_list_plus', 'set' => 'navigation')
    );
    ...
    public static function refreshFilters()
    {
         ...
         *
         *    public static $pageListPlusFilters = array('my_filter', 'my_other_filter');
         *
    ...
    private function installFiltersForExistingPackage($existingPackage)
    {
        ...
        if (!is_array($pageListPlusFilters)) {
            $pageListPlusFilters = array($pageListPlusFilters);
        }
    ...
    public static function uninstallFilters($package)
    {
        ...
        if ($db->getOne($sql, array('aoPageListPlusFilterPlugins', $databaseParameters['database']))) {
