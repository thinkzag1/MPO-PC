...
class Standard extends FilterContract
{
    ...
    public function run()
    {
        if (in_array($this->currentAttributeFilter['handle'], array('cvDatePublic', 'cDateModified'))) {
            $this->runDateFilter();
        } elseif (in_array($this->currentAttributeFilter['handle'], array('cvAuthorUID', 'cvApproverUID', 'p.uID'))) {
            $this->runStandardUserFilter();
        } elseif (in_array($this->currentAttributeFilter['handle'], array('cName', 'cvName'))) {
