...
namespace Concrete\Package\SkybluesofaPageListPlus\Src\PageListPlus\Filter\Contract;
...
class BasicContract extends FilterContract
{
    ...
    public function run()
    {
        if (in_array($this->currentAttributeFilter['filterSelection'], array('past', 'future', 'not_future', 'today_and_future', 'yesterday', 'today', 'tomorrow', 'last_year', 'this_year', 'next_year', 'last_365', 'next_365'))) {
            $this->runDateFilter();
        } else {
            ...
            if (!is_array($this->searchFilters[$this->pageAttributeKeyId])) {
                $this->searchFilters[$this->pageAttributeKeyId] = array($this->searchFilters[$this->pageAttributeKeyId]);
            }
            if (strpos($this->currentAttributeFilter['filterSelection'], 'attribute') !== false) {
                ...
                $this->pageListPlus->filterByClause("(" . $this->currentAttributeFilter['handle'] . " LIKE '%" . $this->currentAttributeFilter['val1'] . "')");
            } elseif (in_array($this->currentAttributeFilter['filterSelection'], array('less_than', 'less_than_allow_negatives'))) {
                $filterClause = $this->currentAttributeFilter['handle'] . "<'" . $this->currentAttributeFilter['val1'] . "'";
                if (strpos($this->currentAttributeFilter['filterSelection'], 'allow_negatives') === false && in_array($this->pageAttributeKeyHandle, array('number'))) {
                    $filterClause .= " && " . $this->currentAttributeFilter['handle'] . ">'0'";
                }
                $this->pageListPlus->filterByClause($filterClause);
            } elseif (in_array($this->currentAttributeFilter['filterSelection'], array('less_than_match', 'less_than_match_allow_negatives'))) {
                if (in_array($this->pageAttributeKeyHandle, array('number'))) {
                ...
                $this->pageListPlus->filterByClause($filterClause);
            } elseif (in_array($this->currentAttributeFilter['filterSelection'], array('less_than_querystring', 'less_than_querystring_allow_negatives'))) {
                ...
                $this->pageListPlus->filterByClause($filterClause);
            } elseif (in_array($this->currentAttributeFilter['filterSelection'], array('less_than_or_equal_to', 'less_than_or_equal_to_allow_negatives'))) {
                $filterClause = $this->currentAttributeFilter['handle'] . "<='" . $this->currentAttributeFilter['val1'] . "'";
                if (strpos($this->currentAttributeFilter['filterSelection'], 'allow_negatives') === false && in_array($this->pageAttributeKeyHandle, array('number'))) {
                    $filterClause .= " && " . $this->currentAttributeFilter['handle'] . ">'0'";
                }
                $this->pageListPlus->filterByClause($filterClause);
            } elseif (in_array($this->currentAttributeFilter['filterSelection'], array('less_than_or_equal_to_match', 'less_than_or_equal_to_match_allow_negatives'))) {
                $filterClause = $this->currentAttributeFilter['handle'] . "<='" . $this->currentAttributeFilter['currentValue'] . "'";
                if (strpos($this->currentAttributeFilter['filterSelection'], 'allow_negatives') === false && in_array($this->pageAttributeKeyHandle, array('number'))) {
                    $filterClause .= " && " . $this->currentAttributeFilter['handle'] . ">'0'";
                }
                $this->pageListPlus->filterByClause($filterClause);
            } elseif (in_array($this->currentAttributeFilter['filterSelection'], array('less_than_or_equal_to_querystring', 'less_than_or_equal_to_querystring_allow_negatives'))) {
                if ($this->currentAttributeFilter['isStandardProperty'])
                    $filterClause = $this->currentAttributeFilter['handle'] . "<='" . $this->currentAttributeFilter['currentValue'] . "'";
                elseif (array_key_exists($this->pageAttributeKeyId, $this->searchFilters) && isset($this->searchFilters[$this->pageAttributeKeyId][0]) && !empty($this->searchFilters[$this->pageAttributeKeyId][0])) {
                    $filterClause = $this->currentAttributeFilter['handle'] . "<='" . $this->searchFilters[$this->pageAttributeKeyId][0] . "'";
                ...
                $this->pageListPlus->filterByClause("(" . $this->currentAttributeFilter['handle'] . "<'" . $this->currentAttributeFilter['val1'] . "' OR " . $this->currentAttributeFilter['handle'] . ">'" . $this->currentAttributeFilter['val2'] . "')");
            } elseif (in_array($this->currentAttributeFilter['filterSelection'], array('between_inclusive_querystring', 'between_exclusive_querystring', 'not_between_inclusive_querystring', 'not_between_exclusive_querystring'))) {
                if ($this->hasSearchValue()) {
                    if (!is_array($this->searchFilters[$this->pageAttributeKeyId])) {
                        $this->searchFilters[$this->pageAttributeKeyId] = array($this->searchFilters[$this->pageAttributeKeyId]);
                    }
                    if (count($this->searchFilters[$this->pageAttributeKeyId]) > 0) {
                        ...
                        $searchComparisons = array(
                            'between_inclusive_querystring' => array(">=","<="),
                            'between_exclusive_querystring' => array(">","<"),
                            'not_between_inclusive_querystring' => array("<=",">="),
                            'not_ between_exclusive_querystring' => array("<",">"),
                        );
