...
class Generator
{
    ...
    public function __construct($settings = array())
    {
        ...
        if (!is_array($this->settings['pageAttributeId'])) {
            $this->settings['pageAttributeId'] = array();
        }
        if (!is_array($this->settings['pageAttributeIdsUsedInSearch'])) {
            $this->settings['pageAttributeIdsUsedInSearch'] = array();
        }
    ...
    private function filterByPageTypeIds()
    {
        ...
        if ($pageTypeIds && count($pageTypeIds) > 0) {
            if (!is_array($pageTypeIds)) {
                $pageTypeIds = array($pageTypeIds);
            }
            $filterPageTypeIds = array();
    ...
    private function filterByPageTemplateIds()
    {
        ...
        if ($pageTemplateIds && count($pageTemplateIds) > 0) {
            if (!is_array($pageTemplateIds)) {
                $pageTemplateIds = array($pageTemplateIds);
            }
    ...
    private function filterByPageThemeIds()
    {
        ...
        if ($pageThemeIds && count($pageThemeIds) > 0) {
            if (!is_array($pageThemeIds)) {
                $pageThemeIds = array($pageThemeIds);
            }
            $filterPageThemeIds = array();
    ...
    private function filterByPageLocation()
    {
        ...
        } elseif (in_array($parentPageId, array('CURRENT_LEVEL', 'PARENT_LEVEL'))) {
            $currentPageId = (isset($this->settings['current_page']) && !is_null($this->settings['current_page'])) ? $this->settings['current_page'] : 0;
            if ($currentPageId > 1) {
    ...
    private function filterByFulltextKeywords($key = 'keyword')
    {
        ...
        if ($this->settings['mysqlFulltextModifier']) {
            if (in_array($this->settings['mysqlFulltextModifier'], array('boolean', 'phrase'))) {
    ...
    {
        $skipBlockTypeHandles = array('youtube', 'video', 'next_previous', 'horizontal_rule', 'search', 'page_list_plus', 'page_list_map_settings', 'geocode_map');
        $content = "";
    ...
    private function setCustomAttributeFilter()
    {
        ...
        $this->settings['pageAttributeIdsUsedInSearch'] = $this->settings['pageAttributeIdsUsedInSearch'] ? $this->settings['pageAttributeIdsUsedInSearch'] : array();
        $this->settings['pageAttributeIdsUsedInSearch'] = is_array($this->settings['pageAttributeIdsUsedInSearch']) ? $this->settings['pageAttributeIdsUsedInSearch'] : array($this->settings['pageAttributeIdsUsedInSearch']);
        ...
        foreach ($attributeFilters as $pageAttributeFilterId => $attributeFilter) {
            ...
            } elseif (in_array($pageAttributeFilterId, array('cID', 'cvName', 'cvDescription', 'cDatePublic', 'cvDatePublic', 'cDateModified', 'cvDateCreated', 'uID', 'cvAuthorUID', 'cvApproverUID'))) {
                $pageAttribute = false;
        ...
        if ($defaults) {
            ...
            foreach ($defaults as $akID => $value) {
                if (empty($value)) {
                    ...
                    $defaults[$akID] = is_array($value) ? $value : array($value);
                }
            }
        } else {
            $defaults = array();
    ...
    private function setupSorting()
    {
        ...
        if (!is_array($this->settings['orderBy'])) {
            $this->settings['orderBy'] = array($this->settings['orderBy']);
        }
    ...
    private function getSorting($order = null)
    {
        ...
        switch ($order) {
            case 'display_asc':
                $sorting = array('column' => 'p.cDisplayOrder', 'direction' => 'asc');
                break;
            case 'display_desc':
                $sorting = array('column' => 'p.cDisplayOrder', 'direction' => 'desc');
                break;
            case 'chrono_asc':
                $sorting = array('column' => 'cvDatePublic', 'direction' => 'asc');
                break;
            case 'chrono_desc':
                $sorting = array('column' => 'cvDatePublic', 'direction' => 'desc');
                break;
            case 'modified_asc':
                $sorting = array('column' => 'cvDateCreated', 'direction' => 'asc');
                break;
            case 'modified_desc':
                $sorting = array('column' => 'cvDateCreated', 'direction' => 'desc');
                break;
            case 'alpha_asc':
                $sorting = array('column' => 'cvName', 'direction' => 'asc');
                break;
            case 'alpha_desc':
                $sorting = array('column' => 'cvName', 'direction' => 'desc');
                break;
            case 'relevance_desc':
                if ($this->isFulltextSearch) {
                    $sorting = array('column' => 'cIndexScore', 'direction' => 'desc');
                } else {
                    $sorting = array('column' => 'p.cDisplayOrder', 'direction' => 'asc');
                }
                break;
            case 'relevance_asc':
                if ($this->isFulltextSearch) {
                    $sorting = array('column' => 'cIndexScore', 'direction' => 'asc');
                } else {
                    $sorting = array('column' => 'p.cDisplayOrder', 'direction' => 'desc');
                }
                break;
            case 'random':
                $sorting = array('column' => 'rand()', 'direction' => 'asc');
                break;
            default:
                //Ideal Code
                ...
                if (isset($columns[$column])) {
                    $sorting = array('column' => $column, 'direction' => $direction);
                } elseif (isset($columns[$column . '_percentage'])) {
                    $sorting = array('column' => $column . '_percentage ', 'direction' => $direction);
                } else {
                    return false;
                }
                break;
