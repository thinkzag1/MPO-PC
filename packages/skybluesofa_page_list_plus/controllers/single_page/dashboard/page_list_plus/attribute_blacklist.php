                ...
                if (isset($_POST['blacklist'])) {
                    ...
                    foreach ($pageAttributes as $pageAttribute) {
                        if (!in_array($pageAttribute->getAttributeKeyID(), $_POST['blacklist'])) {
                            $db->Execute("DELETE FROM aoPageListPlusAttributeBlacklist WHERE akHandle=?", array($pageAttribute->getAttributeKeyHandle()));
                        } else {
                            $q = $db->Execute("SELECT akHandle FROM aoPageListPlusAttributeBlacklist WHERE akHandle=?", array($pageAttribute->getAttributeKeyHandle()));
                            if ($q->FetchRow()) {
                                $db->Execute("UPDATE aoPageListPlusAttributeBlacklist SET blacklist=? WHERE akHandle=?", array(1, $pageAttribute->getAttributeKeyHandle()));
                            } else {
                                $db->Execute("INSERT INTO aoPageListPlusAttributeBlacklist (blacklist, akHandle) VALUES (?, ?)", array(1, $pageAttribute->getAttributeKeyHandle()));
                            }
                        }
                    }
                }
