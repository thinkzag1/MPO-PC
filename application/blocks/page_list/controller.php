...
class CustomPageList extends \Concrete\Core\Page\PageList
{
    ...
    public function filterByTopics($topics)
    {
        ...
        if (is_array($topics))
        {
            ...
            foreach ($topics as $topic) {
                ...
                $topic_id_list .= $treeNodeID . ",";
            }
 
            $topic_id_list = trim($topic_id_list,",");
        }
        else
        {
            if (is_object($topics)) {
        ...
        if ($is_multi)
        {
            $query
                ...
                ->where('atst.treeNodeID IN (' . $topic_id_list . ')')
            ;
        }
        else
        {
    ...
    public function filterByUserIDs($uIDs)
    {
        $uIDs = implode(',',$uIDs);
        $this->query->andWhere('p.uID IN (:uIDs)');
        $this->query->setParameter('uIDs', $uIDs);
    }
...
class Controller extends BlockController
{
    ...
    public function on_start()
    {
        ...
        if($_POST['topicFilter']){
            $session->set('topicFilters', serialize($_POST['topicFilter']));
            //foreach($_POST['topicFilter'] as $topicID){
                //$this->list->filterByTopics($session_topicFilters);
            //}
        } elseif($_POST['form-type'] == 'topic select'){
            $session->set('topicFilters', '');
        }
        $session_topicFilters = unserialize($session->get('topicFilters'));
        if($session_topicFilters){
            $this->list->filterByTopics($session_topicFilters);
        }
        
        // filter by authors
        if($_POST['authorFilter'] || $_GET['authorFilter']){
            if($_POST['authorFilter']){
                $session->set('authorFilters', serialize($_POST['authorFilter']));
            } else {
                $session->set('authorFilters', serialize(array($_GET['authorFilter'])));
            }
            //foreach($_POST['topicFilter'] as $topicID){
                //$this->list->filterByTopics($session_topicFilters);
            //}
        } elseif($_POST['form-type'] == 'user select' || empty($_GET['authorFilter'])){
            $session->set('authorFilters', '');
        }
        $session_authorFilters = unserialize($session->get('authorFilters'));
        if($session_authorFilters){
            //foreach($session_authorFilters as $selUser){
                $this->list->filterByUserIDs($session_authorFilters);
            //}
        }
